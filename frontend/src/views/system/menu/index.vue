<template>
  <div class="app-container">
    ...
    <tree-table v-loading="loading" :data="data" :expand-all="true" :columns="columns" border size="small">
      <el-table-column prop="icon" label="圖示" align="center" width="80px">
      ...
      <el-table-column :show-overflow-tooltip="true" prop="path" label="連結位址"/>
      <el-table-column :show-overflow-tooltip="true" prop="component" label="元件路徑"/>
      <el-table-column prop="iframe" width="100px" label="內部選單">
        <template slot-scope="scope">
          <span v-if="!scope.row.iframe">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
      <el-table-column prop="createTime" label="創建日期">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="150px" align="center">
        <template slot-scope="scope">
          ...
          <el-popover
            ...
            placement="top"
            width="200">
            <p>確定刪除嗎,如果存在下級節點則一併刪除，此操作不能撤銷！</p>
  ...
  methods: {
    ...
    getMenus() {
      getMenusTree().then(res => {
        this.menus = []
        const menu = { id: 0, label: '頂級類目', children: [] }
        menu.children = res
